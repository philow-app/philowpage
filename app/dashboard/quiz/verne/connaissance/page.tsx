...
import { verneConnaissance } from "@/lib/quiz-data"
import { quizService } from "@/lib/quiz-service"

type AnswerStatus = "unknown" | "known" | "unknown_answered"

export default function VerneConnaissancePage() {
  const router = useRouter()
  const questions = verneConnaissance
    ...
    const loadSavedData = async () => {
      try {
        const [savedAnswers, savedPosition] = await Promise.all([
          quizService.loadQuizAnswers('verne', 'connaissance'),
          quizService.loadQuizPosition('verne', 'connaissance')
        ])
  ...
  const handleAnswer = async (status: AnswerStatus) => {
    ...
    try {
      await quizService.saveQuizAnswer('verne', 'connaissance', currentQuestion.id, status)
    } catch (error) {
  ...
  const goToQuestion = async (index: number) => {
    ...
    try {
      await quizService.saveQuizPosition('verne', 'connaissance', index)
    } catch (error) {
  ...
  const nextQuestion = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        await quizService.saveQuizPosition('verne', 'connaissance', newIndex)
      } catch (error) {
        console.error('Erreur lors de la sauvegarde de position:', error)
      }
      setShowAnswer(false)
  ...
  const resetQuiz = async () => {
    setIsResetting(true)
    try {
      await quizService.resetQuiz('verne', 'connaissance')
      setAnswers({})
  ...
  return (
    <DashboardLayout>
      <div className="flex h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-1 h-12 bg-gradient-to-b from-blue-400 to-cyan-500 rounded-full"></div>
                  <div>
                    <h1 className="text-3xl font-light text-gray-800 tracking-tight">
                      Vingt mille lieues sous les mers
                    </h1>
                    <p className="text-sm text-gray-500 font-normal mt-1">Jules Verne • Test de Connaissance</p>
                  </div>
                </div>
              </div>
            ...
            <Card className="max-w-4xl mx-auto shadow-xl border-0 bg-white/95 backdrop-blur-sm">
              <CardHeader className="pb-8">
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div 
                    className="bg-gradient-to-r from-blue-400 to-cyan-500 h-1.5 rounded-full transition-all duration-700 ease-out" 
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-4 justify-center">
                      <Button 
                        ...
                        className="px-8 py-3 text-base font-medium bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 disabled:opacity-50"
                      >
                        Question précédente
                      </Button>
                      <Button 
        ...
        <div className="w-80 bg-white/95 backdrop-blur-sm border-l border-blue-200 p-6 shadow-lg h-screen overflow-y-auto">
          <div className="space-y-6">
              ...
              {questions.map((question, index) => (
                <button
                  ...
                  className={`h-9 w-9 text-xs font-bold rounded-lg transition-all duration-300 cursor-pointer ${getQuestionStatus(question.id)} ${
                    index === currentQuestionIndex ? "ring-3 ring-blue-300 ring-offset-1 scale-110 shadow-lg" : "hover:scale-105"
                  }`}
                >
                  {question.id}
                </button>
