...
import { verneVocabulaire } from "@/lib/quiz-data"
...
export default function VerneVocabulairePage() {
  const router = useRouter()
  const questions = verneVocabulaire
    ...
    const loadSavedData = async () => {
      try {
        const [savedAnswers, savedPosition] = await Promise.all([
          quizService.loadQuizAnswers('verne', 'vocabulaire'),
          quizService.loadQuizPosition('verne', 'vocabulaire')
        ])
  ...
  const handleAnswer = async (selectedIndex: number) => {
    ...
    try {
      ...
      await quizService.saveQuizAnswer('verne', 'vocabulaire', currentQuestion.id, supabaseStatus)
    } catch (error) {
  ...
  const goToQuestion = async (index: number) => {
    ...
    try {
      await quizService.saveQuizPosition('verne', 'vocabulaire', index)
    } catch (error) {
  ...
  const nextQuestion = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        await quizService.saveQuizPosition('verne', 'vocabulaire', newIndex)
      } catch (error) {
        console.error('Erreur lors de la sauvegarde de position:', error)
      }
  ...
  const resetQuiz = async () => {
    setIsResetting(true)
    try {
      await quizService.resetQuiz('verne', 'vocabulaire')
      setAnswers({})
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 font-normal mt-1">Jules Verne • Test de Vocabulaire</p>
                  </div>
                </div>
              </div>
            ...
            <Card className="max-w-4xl mx-auto shadow-xl border-0 bg-white/95 backdrop-blur-sm">
              ...
              <CardContent className="space-y-8">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                  <p className="text-sm text-blue-700 font-medium">📚 Voir les fiches de révisions pour des définitions plus détaillées</p>
                </div>
                ...
                {showAnswer && (
                  <div className="flex gap-4 justify-center">
                    <Button 
                      ...
                      className="px-8 py-3 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 disabled:opacity-50"
                    >
                      ← Question précédente
                    </Button>
                      <Button 
                        ...
                        className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white px-8 py-3 rounded-xl shadow-lg"
                      >
                        Question suivante →
                      </Button>
                    </div>
