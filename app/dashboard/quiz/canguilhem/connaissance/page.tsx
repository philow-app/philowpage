...
import { canguilhemConnaissance } from "@/lib/quiz-data"
import { quizService } from "@/lib/quiz-service"

type AnswerStatus = "unknown" | "known" | "unknown_answered"

export default function CanguilhemConnaissancePage() {
  const router = useRouter()
  const questions = canguilhemConnaissance
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        const savedAnswers = await quizService.loadQuizAnswers('canguilhem', 'connaissance')
        setAnswers(savedAnswers)

        // Charger la position sauvegardée
        const savedPosition = await quizService.loadQuizPosition('canguilhem', 'connaissance')
        if (savedPosition !== null && savedPosition >= 0 && savedPosition < questions.length) {
  ...
  const handleAnswer = async (status: AnswerStatus) => {
    ...
    try {
      await quizService.saveQuizAnswer('canguilhem', 'connaissance', currentQuestion.id, status)
    } catch (error) {
  ...
  const goToQuestion = async (index: number) => {
    ...
    try {
      await quizService.saveQuizPosition('canguilhem', 'connaissance', index)
    } catch (error) {
  ...
  const nextQuestion = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        await quizService.saveQuizPosition('canguilhem', 'connaissance', newIndex)
      } catch (error) {
        console.error('Erreur lors de la sauvegarde de position:', error)
      }
      setShowAnswer(false)
  ...
  const resetQuiz = async () => {
    setIsResetting(true)
    try {
      await quizService.resetQuiz('canguilhem', 'connaissance')
      setAnswers({})
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-1 h-12 bg-gradient-to-b from-purple-400 to-violet-500 rounded-full"></div>
                  <div>
                    <h1 className="text-3xl font-light text-gray-800 tracking-tight">
                      La Connaissance de la vie
                    </h1>
                    <p className="text-sm text-gray-500 font-normal mt-1">Georges Canguilhem • Test de Connaissance</p>
                  </div>
                </div>
              </div>
            ...
            <Card className="max-w-4xl mx-auto shadow-xl border-0 bg-white/95 backdrop-blur-sm">
              <CardHeader className="pb-8">
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div 
                    className="bg-gradient-to-r from-purple-400 to-violet-500 h-1.5 rounded-full transition-all duration-700 ease-out" 
              ...
              <CardContent className="space-y-8">
                ...
                {!showAnswer ? (
                  ...
                  <div className="space-y-6">
                    <div className="bg-gradient-to-r from-purple-50 to-violet-50 border-l-4 border-purple-500 p-6 rounded-r-xl shadow-inner">
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-6 h-6 bg-purple-500 rounded-full flex items-center justify-center">
                          <span className="text-white text-xs font-bold">✓</span>
                        </div>
                        <h4 className="font-semibold text-purple-800">Réponse</h4>
                      </div>
                    ...
                    <div className="flex gap-4 justify-center">
                      ...
                      <Button 
                        ...
                        className="bg-gradient-to-r from-purple-500 to-violet-500 hover:from-purple-600 hover:to-violet-600 text-white px-8 py-3 rounded-xl shadow-lg"
                      >
                        Question suivante →
                      </Button>
                    </div>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-green-600">Maîtrisées :</span>
                  <span className="font-bold text-green-600 text-lg">{stats.known}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-red-600">À revoir :</span>
                  <span className="font-bold text-red-600 text-lg">{stats.unknown}</span>
                </div>
              </div>
              ...
              <div className="text-center">
                <span className="text-sm text-gray-600">
                  {Math.round((stats.known / stats.total) * 100)}% maîtrisé
                </span>
              </div>
            </div>
